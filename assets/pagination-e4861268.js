import{S as V,a as y,P as Q}from"./vendor-bb89de92.js";(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))s(t);new MutationObserver(t=>{for(const c of t)if(c.type==="childList")for(const i of c.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&s(i)}).observe(document,{childList:!0,subtree:!0});function o(t){const c={};return t.integrity&&(c.integrity=t.integrity),t.referrerpolicy&&(c.referrerPolicy=t.referrerpolicy),t.crossorigin==="use-credentials"?c.credentials="include":t.crossorigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function s(t){if(t.ep)return;t.ep=!0;const c=o(t);fetch(t.href,c)}})();function E(e){const a=JSON.parse(localStorage.getItem("cart"))||[],o=Array(a).flat(1/0);o.push(e),a.includes(e)||localStorage.setItem("cart",JSON.stringify(o))}function O(e){const a=JSON.parse(localStorage.getItem("cart")),o=Array(a).flat(1/0),s=o.indexOf(e);o.splice(s,1),localStorage.setItem("cart",JSON.stringify(o))}function l(e){const a=JSON.parse(localStorage.getItem("cart"));return Array(a).flat(1/0).some(s=>s===e)}const X="/santa-sleigh-team-2/assets/fruitCart@2x-44c39800.png",Z="/santa-sleigh-team-2/assets/check-mark-35f2f86f.png",f=document.querySelector(".qty-card"),J=JSON.parse(localStorage.getItem("cart"))||[],ee=J.length>0?J.length:0;let te=ee;function b(e){f.textContent=String(e)}b(te);const n="/santa-sleigh-team-2/assets/icons-b3cb9da3.svg",h=document.querySelector(".js-modal"),se=document.querySelector(".modal-content"),v=document.querySelector(".js-modal-subscription"),z=document.querySelector(".subscription-modal-content"),w=document.querySelector(".js-modal-order"),ae=document.querySelector(".subscription-order-content"),S=document.querySelector("body");function x(){S.style.overflow="hidden"}function P(){S.style.overflow="auto"}async function H(e){h.classList.toggle("is-hidden"),await pe(e).then(a=>{const{img:o,name:s,desc:t,category:c,size:i,popularity:d,price:r,is10PercentOff:u}=a,M=t.split("");se.innerHTML=`<div class='image-wrap'>
  ...
  `,window.addEventListener("keydown",e);function e(s){s.key==="Escape"&&w.classList.add("is-hidden")}window.onclick=function(s){s.target===w&&w.classList.add("is-hidden")};const a=document.querySelector(".modal-close-btn");a.addEventListener("click",o);function o(s){s.currentTarget===a&&w.classList.add("is-hidden")}}const I=document.querySelector(".list-product"),_=document.querySelector(".form-search"),N=document.querySelector(".error"),Y=document.querySelector("#selected");N.style.display="none";let q,k;_.addEventListener("submit",ce);const G=window.innerWidth;let L;G<=767?L=6:G<=1239?L=8:L=9;function ce(e){e.preventDefault();const{search:a}=e.currentTarget.elements;q=a.value,localStorage.setItem("SaveFilters",JSON.stringify(q)||null),m()}_.elements.search.value=localStorage.getItem("savetext");m();Y.addEventListener("change",ne);function ne(e){e.target.value;const a=e.target.value;k=a,k=a,localStorage.setItem("SaveCategpries",JSON.stringify(k)||null),m()}_.elements.selecteds.value=localStorage.getItem("saveselected");function ie(){ue().then(e=>{const a=e.map(o=>`<option value="${String(o).replace("_","_").replace("&","%26")}">
    ...
    </li>`).join("")}async function be(){const e=await de(),a=e.reduce((s,t)=>(t.name.length>10?s.push(t.name.slice(0,10)+"..."):s.push(t.name),s),[]);R.innerHTML=fe(e,a),R.addEventListener("click",s=>{const t=s.target.closest(".card_item"),c=s.target.closest(".basket");if(t&&!c){const i=t.dataset.id;H(i).then(x)}}),document.querySelectorAll(".basket").forEach(s=>{s.addEventListener("click",ye);const t=s.dataset.id;l(t)&&(s.setAttribute("disabled","true"),s.innerHTML=`
        <svg class="basket-icon-check" width="18" height="18">
          <use href="${n}#icon-check"></use>
        </svg>
    ...
    </li>`).join("");a.innerHTML=o,U.appendChild(a)}function Se(e){const a=e.target.closest(".product-content"),o=e.target.closest(".popular_basket");if(a&&!o){const s=a.dataset.id;H(s).then(x)}}U.addEventListener("click",Se);ve();document.querySelector(".pagination");let $=0;const C=window.innerWidth;function ke(e){return e<=767?2:4}C<=767?$=6:C<=1239?$=8:$=9;let T=1;function $e(e){T=1;const a={totalItems:e.totalPages*10,visiblePages:ke(C),template:{page:'<a href="#" id="page-btn" class="tui-page-btn">{{page}}</a>',currentPage:'<strong id="is-selected" class="tui-page-btn tui-is-selected">{{page}}</strong>',moveButton:'<a href="#" id="arrow-btn" class="tui-page-btn tui-{{type}} custom-class-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></a>',disabledMoveButton:'<span id="arrow-btn" class="tui-page-btn tui-is-disabled tui-{{type}} custom-class-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></span>',moreButton:'<a href="#" id="page-btn" class="tui-page-btn tui-{{type}}-is-ellip custom-class-{{type}}"><span class="tui-ico-ellip">...</span></a>'}};new Q("pagination",a).on("afterMove",function(s){T=s.page,D(T,$).then(t=>{K(t.results)}).catch(t=>{throw new Error(t)})})}D(T,$).then(e=>{$e(e)}).catch(e=>{throw new Error(e)});export{pe as g,b as h,Te as o,O as r,n as s};
//# sourceMappingURL=pagination-e4861268.js.map
