import{a as y,P as V}from"./vendor-b592f4c5.js";(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))s(t);new MutationObserver(t=>{for(const c of t)if(c.type==="childList")for(const i of c.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&s(i)}).observe(document,{childList:!0,subtree:!0});function o(t){const c={};return t.integrity&&(c.integrity=t.integrity),t.referrerpolicy&&(c.referrerPolicy=t.referrerpolicy),t.crossorigin==="use-credentials"?c.credentials="include":t.crossorigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function s(t){if(t.ep)return;t.ep=!0;const c=o(t);fetch(t.href,c)}})();function E(e){const a=JSON.parse(localStorage.getItem("cart"))||[],o=Array(a).flat(1/0);o.push(e),a.includes(e)||localStorage.setItem("cart",JSON.stringify(o))}function _(e){const a=JSON.parse(localStorage.getItem("cart")),o=Array(a).flat(1/0),s=o.indexOf(e);o.splice(s,1),localStorage.setItem("cart",JSON.stringify(o))}function l(e){const a=JSON.parse(localStorage.getItem("cart"));return Array(a).flat(1/0).some(s=>s===e)}const Q="/fson-95-project-team_2/assets/fruitCart@2x-44c39800.png",X="/fson-95-project-team_2/assets/check-mark-35f2f86f.png",f=document.querySelector(".qty-card"),J=JSON.parse(localStorage.getItem("cart"))||[],Z=J.length>0?J.length:0;let ee=Z;function b(e){f.textContent=String(e)}b(ee);const n="/fson-95-project-team_2/assets/icons-b3cb9da3.svg",h=document.querySelector(".js-modal"),te=document.querySelector(".modal-content"),v=document.querySelector(".js-modal-subscription"),z=document.querySelector(".subscription-modal-content"),w=document.querySelector(".js-modal-order"),se=document.querySelector(".subscription-order-content"),S=document.querySelector("body");function x(){S.style.overflow="hidden"}function P(){S.style.overflow="auto"}async function O(e){h.classList.toggle("is-hidden"),await ue(e).then(a=>{const{img:o,name:s,desc:t,category:c,size:i,popularity:d,price:r,is10PercentOff:u}=a,M=t.split("");te.innerHTML=`<div class='image-wrap'>
  ...
  `,window.addEventListener("keydown",e);function e(s){s.key==="Escape"&&w.classList.add("is-hidden")}window.onclick=function(s){s.target===w&&w.classList.add("is-hidden")};const a=document.querySelector(".modal-close-btn");a.addEventListener("click",o);function o(s){s.currentTarget===a&&w.classList.add("is-hidden")}}const I=document.querySelector(".list-product"),H=document.querySelector(".form-search"),N=document.querySelector(".error"),Y=document.querySelector("#selected");N.style.display="none";let q,k;H.addEventListener("submit",oe);const G=window.innerWidth;let L;G<=767?L=6:G<=1239?L=8:L=9;function oe(e){e.preventDefault();const{search:a}=e.currentTarget.elements;q=a.value,localStorage.setItem("SaveFilters",JSON.stringify(q)||null),m()}H.elements.search.value=localStorage.getItem("savetext");m();Y.addEventListener("change",ce);function ce(e){e.target.value;const a=e.target.value;k=a,k=a,localStorage.setItem("SaveCategpries",JSON.stringify(k)||null),m()}H.elements.selecteds.value=localStorage.getItem("saveselected");function ne(){de().then(e=>{const a=e.map(o=>`<option value="${String(o).replace("_","_").replace("&","%26")}">
    ...
    </li>`).join("")}async function fe(){const e=await le(),a=e.reduce((s,t)=>(t.name.length>10?s.push(t.name.slice(0,10)+"..."):s.push(t.name),s),[]);R.innerHTML=me(e,a),R.addEventListener("click",s=>{const t=s.target.closest(".card_item"),c=s.target.closest(".basket");if(t&&!c){const i=t.dataset.id;O(i).then(x)}}),document.querySelectorAll(".basket").forEach(s=>{s.addEventListener("click",be);const t=s.dataset.id;l(t)&&(s.setAttribute("disabled","true"),s.innerHTML=`
        <svg class="basket-icon-check" width="18" height="18">
          <use href="${n}#icon-check"></use>
        </svg>
    ...
    </li>`).join("");a.innerHTML=o,U.appendChild(a)}function we(e){const a=e.target.closest(".product-content"),o=e.target.closest(".popular_basket");if(a&&!o){const s=a.dataset.id;O(s).then(x)}}U.addEventListener("click",we);ye();document.querySelector(".pagination");let $=0;const C=window.innerWidth;function Se(e){return e<=767?2:4}C<=767?$=6:C<=1239?$=8:$=9;let T=1;function ke(e){T=1;const a={totalItems:e.totalPages*10,visiblePages:Se(C),template:{page:'<a href="#" id="page-btn" class="tui-page-btn">{{page}}</a>',currentPage:'<strong id="is-selected" class="tui-page-btn tui-is-selected">{{page}}</strong>',moveButton:'<a href="#" id="arrow-btn" class="tui-page-btn tui-{{type}} custom-class-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></a>',disabledMoveButton:'<span id="arrow-btn" class="tui-page-btn tui-is-disabled tui-{{type}} custom-class-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></span>',moreButton:'<a href="#" id="page-btn" class="tui-page-btn tui-{{type}}-is-ellip custom-class-{{type}}"><span class="tui-ico-ellip">...</span></a>'}};new V("pagination",a).on("afterMove",function(s){T=s.page,j(T,$).then(t=>{K(t.results)}).catch(t=>{throw new Error(t)})})}j(T,$).then(e=>{ke(e)}).catch(e=>{throw new Error(e)});export{ue as g,b as h,Le as o,_ as r,n as s};
//# sourceMappingURL=pagination-ba5d2204.js.map
